...
\begin{document}
	...
	\section{Voorbeelden en $n=1$}\label{sec:vb}
	De eerste negentien congruente getallen zijn: $5$, $6$, $7$, $13$, $14$, $15$, $21$, $22$, $23$, $29$, $30$, $31$, $34$, $37$, $38$, $39$, $41$, $46$, $47$, zie \cite{Coates}[p.19]. We zien dus dat bijvoorbeeld $n=1$ geen congruent getal is. Daar zullen we een bewijs voor geven, nadat we hieronder drie voorbeelden weergeven van driehoeken die corresponderen met een congruent getal.\\
	\\
	...
	We zullen hieronder bewijzen dat $n=1$ geen congruent getal is. \textcolor{red}{Je hebt hier een tweede zin die het bewijs voor $n=1$ aankondigt. Deze lijkt erg op, en komt erg snel na de eerdere zin waarin je dat doet (namelijk de laatste zin van de eerste alinea). Probeer iets als: 'Alvorens te bewijzen dat...' of iets in die geest.} Daarvoor bekijken we eerst een aantal benodigde stellingen en opmerkingen. Ook hebben we het begrip ``relatief priem" nodig: als twee getallen relatief priem zijn, wil dat zeggen dat ze geen factoren gemeen hebben, oftewel hun grootste gemene deler is 1.
	...
	We moeten nu ook nog iets weten over modulo rekenen. \textcolor{red}{Benoem specifieker wat je gaat doen. Of maak er een aankondigende zin van.}
	...
	\begin{proof}
		...
		\end{equation}
		voor oneven positieve gehele getallen $t$ en $u$, waarvoor geldt dat deze ook relatief priem zijn. We gebruiken nu opmerking \ref{1:hulpMod}; omdat $u$ oneven is, geldt er dat $u^2 \equiv 1 \pmod{4}$, dus $r^2 - s^2 \equiv 1 \pmod{4}$. Hieruit volgt dat $r$ oneven is, dus is $s$ even. Namelijk: Er \textcolor{red}{Geen hoofdletter} moet gelden dat $r$ en $s$ verschillende pariteit hebben, want anders geldt er dat $r^2 - s^2 \equiv 0 \pmod{4}$. Stel nu dat $r$ is even, dan is $s$ oneven. Er geldt nu dat $r^2 \equiv 0 \pmod{4}$ en $s^2 \equiv 1 \pmod{4}$. Dit levert op: $r^2 - s^2 \equiv -1 \equiv 3 \pmod{4}$ en dat kan niet. We weten dus dat $r$ oneven is en dat $s$ even is. Nu lossen we  (\ref{1:r}) op voor $r^2$. Dit geeft ons:
		\begin{equation*}
	...
	\end{proof}
	\textcolor{red}{(Geen commentaar waar je iets mee moet gaan doen, maar ik wil het toch maar even gezegd hebben.) Jezus, wat is dit bewijs lang.}\\
	\textcolor{red}{Commentaar waar je eventueel wel iets mee kan doen: het bewijs van $n=1$ tot een apart hoofdstuk maken, en dat na het 'Congruente getallen vinden' doen. (Eventueel ook na het hoofdstuk 'Presentaties', maar dat is persoonlijke voorkeur.) Lijkt mij een iets logischere volgorde, zeker gezien de laatste zin van het hoofdstuk 'Congruente getallen vinden'. Je hoeft dan alleen maar iets te veranderen in de eerste alinea van het kopje voorbeelden.}
	
	
	\section{Congruente getallen vinden}\label{sec:suus}
	Voor dit hoofdstuk hebben we gebruik gemaakt van \cite{Koblitz}[p.3-4].\\
	
	We weten dat een congruent getal $n$ een geheel getal is\textcolor{red}{,} dat de oppervlakte is van een driehoek met rationale zijden. We willen nu graag weten welke getallen $m\in\NN$ congruent zijn. Als we dat willen doen met de kennis van nu, moeten we voor elke $m$ gaan kijken of we een driehoek kunnen vinden met zijden in $\QQ$ zodat de oppervlakte van deze driehoek gelijk is aan $m$. Het is niet moeilijk om te bedenken dat dat een onmogelijke \textcolor{red}{Ik geloof dat je hier 'onbegonnen' bedoelt, in plaats van 'onmogelijke'} zaak is, zelfs als we de computer laten rekenen. Dus moeten we op zoek gaan naar een manier om het aantal mogelijke getallen $m\in\NN$ te verminderen. Om te kijken hoe we dat kunnen doen, kijken we eerst naar de volgende driehoek:
	\begin{figure}[h!]
		\centering
	...
	\begin{theorem}\label{def:tunnell}
		...
		\begin{align}
			...
			\notag \text{(respectievelijk \textcolor{red}{Je kunt de haken weglaten}} \\
