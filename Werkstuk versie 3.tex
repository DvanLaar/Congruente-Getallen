...
\begin{document}
	...
	\begin{definition}\label{def:algebra}
		Een positief geheel getal $n$ heet een congruent getal als er een $\delta \in \QQ_{>0}$ bestaat zodanig dat
		\begin{equation*}
	...
	\begin{figure}[h!]
		...
		\draw (S) node[auto]{$x,y,z\in\QQ_{>0}$};
	...
	\begin{figure}[h!]
		...
		\coordinate (S) at (2.0, 0.7);
		\draw (S) node[auto]{$sx,sy,sz\in\QQ_{>0}$};
