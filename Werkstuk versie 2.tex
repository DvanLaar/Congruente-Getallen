...
\begin{document}	
	...
	\section{Voorbeelden}
	We zullen eerst aantonen dat $m=1$ en $m=2$ geen congruente getallen zijn. We nemen daarvoor de volgende rechthoekige driehoek:
	\begin{figure*}[h!]
		\centering
		...
		\end{tikzpicture}
	\end{figure*}\\
	Als $m=1$ een congruent getal is, moet er volgens definitie (\ref{def:meetkunde}) gelden dat er $\frac{1}{2}xy=1$ en $x,y,z\in\QQ$. Hieruit kunnen we afleiden dat:
	\begin{equation}
	xy=2
	\end{equation}
	Waaruit volgt dat:
	\begin{align}
	\label{vgl:x} x=\frac{2}{y}\\
	\label{vgl:y} y=\frac{2}{x}
	\end{align}
	We weten door de stelling van Pythagoras dat er moet gelden $x^2+y^2=z^2$. Als we de vergelijkingen (\ref{vgl:x}) en (\ref{vgl:y}) invullen, krijgen we:
	\begin{align}
	\left(\frac{2}{y}\right)^2 + \left(\frac{2}{x}\right)^2 &= z^2\\
	\Rightarrow \sqrt{\frac{2^2}{y^2} + \frac{2^2}{x^2}} &= z\\
	\Rightarrow \sqrt{\frac{4x^2 + 4y^2}{x^2y^2}} &= z\\
	\Rightarrow \frac{\sqrt{4x^2 + 4y^2}}{xy} &= z
	\end{align} 
	
		
	%Geef een bewijs met n=1 of n=2
	%Geef andere voorbeelden
	Hieronder volgen drie voorbeelden van congruente getallen, volgens definitie \ref{def:meetkunde}. Het kleinste congruente getal is $5$. Dat is de driehoek met de volgende zijden: {\color{red}bewering beargumenteren?}
	\begin{figure*}[h!]
		\centering
		\begin{tikzpicture}[xscale=4, yscale=2]
		\coordinate (O) at (0,0);
